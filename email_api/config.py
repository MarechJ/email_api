...
def _exit_if_no_providers(config):
    if PROVIDERS_KEY not in config:
        _LOG.critical(
            'The configuration must have a "%s" secion', PROVIDERS_KEY
        )
        exit(1)
...
def load_config(path):
    if path and Path(path).exists():
        try:
            ...
            _LOG.error("Invalid YAML file %s", path)
            exit(1)

    # Fallback on default conf
    _LOG.info("No configuration found at %s.", path)
    _LOG.info(
        "You need to input you password to decypher providers API Key"
    )
    ...
    for prov in providers:
        ...
        if prov.nickname not in conf \
           ...
           or not conf[prov.nickname]['key']:
            ...
            _LOG.critical(
                """%s provider not found or improper in the configuration.
                Please add to the config:
                providers:
                \t%s:
                \t\tuser: "yourusername"
                \t\tkey: "yourkey"
